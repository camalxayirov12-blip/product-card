...
function reverseAnyArray(arr) {
    // Используем spread [...arr], чтобы не испортить оригинальный массив
    return [...arr].reverse();
}

console.log("Перевернутые числа:", reverseAnyArray(numbers));
console.log("Перевернутый тюнинг:", reverseAnyArray(m5TuningParts));


// --- Уровень 2 ---

// 7. Комментарии, почта которых содержит ".com"
const comEmails = socialComments.filter(comment => comment.email.endsWith('.com'));
console.log("Почты .com:", comEmails);

// 8. Изменение postId в зависимости от id
// id <= 5 -> postId: 2, остальные -> postId: 1
const updatedCommentsPostId = socialComments.map(comment => ({
    ...comment,
...
// 12. Приведение массива почт к строке через join
const emailsString = emailsMap.join(', ');
console.log("Строка всех почт:", emailsString);

// Дополнительно через toString (как в задании)
console.log("Через toString:", emailsMap.toString());